...
knitr::opts_knit$set(root.dir = "~/R/parvo-b19-modeling/")
```
***
```{r}
## Load model parameters
source("R Tools/01_params_functions.R")
l_params_all <- load_params()
v_names_str <- c("Status Quo", "Screening", "Vaccination", "Vaccination and Screening")
n_str <- length(v_names_str)

## Load model
source("R Tools/02_screen_vax_model_functions.R")
                            ...
                            ifelse(parameter == "p_inf", "Infection Rate",
                                   ifelse(parameter == "p_vax", "Vaccination Coverage",
                                          ifelse(parameter == "p_vax_eff", "Vaccine Effectiveness",
                                                 parameter))))) %>%
  mutate(Intervention = ifelse(strategy == "Status.Quo", "Status Quo",
...
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "owsa_stillbirths.png")
...
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "owsa_b19deaths.png")
...
plot(owsa_results$owsa_Transfusions) +
  ...
  geom_vline(data = intercepts, aes(xintercept = xint), linetype = "dashed") +
  theme(legend.position = "bottom") 
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "owsa_transfusion.png")
...
df_owsa_all$Intervention <- factor(df_owsa_all$Intervention,
...
ggplot(data = df_owsa_all, aes(x = param_val, y = outcome_val, color = Intervention)) +
  geom_line(size = 1.2) +
  facet_grid(outcome~parameter, labeller = label_wrap_gen(18),
             scales = "free") +
  ylab("Incidence per 100,000 pregnancies") +
  ...
  geom_vline(data = intercepts, aes(xintercept = xint), linetype = "dashed") +
  theme(legend.position = "bottom") 
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "owsa_all.png",
...
heat_deaths_ii
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "twsa_imm_inf_deaths_sq_test.png",
       height = 7, width = 7)
...
heat_fd_ii
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "twsa_imm_inf_fd_sq_test.png",
       height = 7, width = 7)
...
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "twsa_imm_inf_trans_sq_test.png",
       height = 7, width = 7)
...
heat_deaths_ii
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "twsa_imm_inf_deaths_sq_vax.png",
       height = 7, width = 7)
...
heat_fd_ii
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "twsa_imm_inf_fd_sq_vax.png",
       height = 7, width = 7)
...
heat_transfusions_ii
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "twsa_imm_inf_trans_sq_vax.png",
       height = 7, width = 7)
...
heat_deaths_ii
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "twsa_imm_inf_deaths_sq_testvax.png",
       height = 7, width = 7)
...
heat_fd_ii
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "twsa_imm_inf_fd_sq_testvax.png",
       height = 7, width = 7)
...
heat_transfusions_ii
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "twsa_imm_inf_trans_sq_testvax.png",
       height = 7, width = 7)


### Preferred strategy (vaccine coverage and effectiveness)
...
# plot preferred strategy as a function of the two parameters varied in the two-way DSA
...
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "twsa_preferred.png",
       height = 5, width = 9)
```
...
df_long_sb <- df_psa_stillbirth %>%
  ...
  pivot_longer(cols = c(screening_inc, vax_inc, vax_screening_inc), names_to = "Intervention", values_to = "Incremental")  %>%
...
df_long_psa_all <- rbind(df_long_sb, df_long_fd, df_long_trans) %>%
  mutate(Intervention = ifelse(Intervention == "vax_screening_inc", "Screening + Vaccination", 
                           ifelse(Intervention == "screening_inc", "Screening",
                                  ifelse(Intervention == "vax_inc", "Vaccination", Intervention))))
ggplot(data = df_long_psa_all, aes(x = Incremental, fill = Intervention)) +
  geom_density() +
  facet_grid(Intervention~Outcome, labeller = label_wrap_gen(18),
             scales = "free_y")
...
ggplot(data = df_scatter_sb, aes(x = screening_inc, y = vax_inc)) + 
  ...
  guides(color=guide_legend(title="Preferred Intervention")) +
  annotate("text", x = c(3, 30), y = c(60, 5), label = c("93.95%", "6.05%"))
ggsave(path = "R Tools/Results/HyVSS Plots", filename = "psa_preferred_sb.png",
       height = 5, width = 7)
